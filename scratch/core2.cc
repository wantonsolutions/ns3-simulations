...
main (int argc, char *argv[])
{
  ...
  NodeContainer switches2;
  switches2.Create (SWITCHES);
  ...
  NodeContainer nc_core2switch2[SWITCHES];
  NetDeviceContainer ndc_core2switch2[SWITCHES];
  
  for (int s = 0; s < SWITCHES; s++) {
      nc_core2switch2[s] = NodeContainer(core.Get(0), switches2.Get(s));
      nc_core2switch[s] = NodeContainer(switches2.Get(s), switches.Get(s));
      //switches to node
  ...
  for (int s = 0; s < SWITCHES; s++ ){
      //core to switch
      ndc_core2switch2[s] = pointToPoint.Install(nc_core2switch2[s]);
  ...
  stack.Install (switches2);
  stack.Install (core);

  Ipv4AddressHelper address;
  Ipv4InterfaceContainer node_interfaces[NODES*SWITCHES];
  Ipv4InterfaceContainer switch_interfaces[SWITCHES];
  Ipv4InterfaceContainer switch_interfaces2[SWITCHES];

  for (int s = 0; s < SWITCHES; s++) {
      std::stringstream saddr2;
      saddr2 << "10." << s+100 << ".0.0";
      address.SetBase(saddr2.str().c_str(), "255.255.0.0");
      switch_interfaces2[s] = address.Assign(ndc_core2switch2[s]);
  ...
  for (int client = 0; client < CLIENTS; client++) {
      ...
      echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
      echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
      echoClient.SetAttribute ("PacketSize", UintegerValue (1020 + client));
