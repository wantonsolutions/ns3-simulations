...
main (int argc, char *argv[])
{
  ...
  for (int server = 0; server < SERVERS; server ++ ){
      ApplicationContainer serverApps = echoServer.Install (bottomNodes.Get (server));
  ...
  for (int client = 0; client < CLIENTS; client++) {
      ...
      UdpEchoClientHelper echoClient (interfaces[client%SERVERS].GetAddress (1), 9);
      echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
      echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
      echoClient.SetAttribute ("PacketSize", UintegerValue (1024));


      ApplicationContainer clientApps = echoClient.Install (bottomNodes.Get ((SERVERS)+client));
