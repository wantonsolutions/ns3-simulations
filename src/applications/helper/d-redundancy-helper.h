 ...
 */
#ifndef D_REDUNDANCY_HELPER_H
#define D_REDUNDANCY_HELPER_H
 ...
 */
class DRedundancyServerHelper
{
public:
  /**
   * Create DRedundancyServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   * TODO update
   */
  DRedundancyServerHelper (uint16_t port, Address* ips, uint8_t parallel);
  ...
  /**
   * Create a DRedundancyServerApplication on the specified Node.
   *
  ...
  /**
   * Create a DRedundancyServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::DRedundancyServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an DRedundancyServer will be installed.
 ...
 */
class DRedundancyClientHelper
{
public:
  /**
   * Create DRedundancyClientHelper which will make life easier for people trying
   ...
   */
 //TODO update
  DRedundancyClientHelper (uint16_t port, Address* addresses, uint8_t parallel);
  /**
   * Create DRedundancyClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  DRedundancyClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a DRedundancyClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be DRedundancyClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the DRedundancyClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the DRedundancyClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::DRedundancyClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an DRedundancyClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
