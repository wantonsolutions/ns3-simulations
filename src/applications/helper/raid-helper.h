 ...
 */
#ifndef RAID_HELPER_H
#define RAID_HELPER_H
 ...
 */
class RaidServerHelper
{
public:
  /**
   * Create RaidServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   * TODO update
   */
  RaidServerHelper (uint16_t port, Address* ips, uint8_t parallel);
  ...
  /**
   * Create a RaidServerApplication on the specified Node.
   *
  ...
  /**
   * Create a RaidServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::RaidServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an RaidServer will be installed.
 ...
 */
class RaidClientHelper
{
public:
  /**
   * Create RaidClientHelper which will make life easier for people trying
   ...
   */
 //TODO update
  RaidClientHelper (uint16_t port, Address* addresses, uint8_t parallel);
  /**
   * Create RaidClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a RaidClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be RaidClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the RaidClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the RaidClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::RaidClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an RaidClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
