 ...
 */
#ifndef UDP_ECHO_REPORT_HELPER_H
#define UDP_ECHO_REPORT_HELPER_H
 ...
 */
class UdpEchoServerReportHelper
{
public:
  /**
   * Create UdpEchoServerReportHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpEchoServerReportHelper (uint16_t port);
  ...
  /**
   * Create a UdpEchoServerReportApplication on the specified Node.
   *
  ...
  /**
   * Create a UdpEchoServerReportApplication on specified node
   *
...
private:
  /**
   * Install an ns3::UdpEchoServerReport on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpEchoServerReport will be installed.
 ...
 */
class UdpEchoClientReportHelper
{
public:
  /**
   * Create UdpEchoClientReportHelper which will make life easier for people trying
   ...
   */
  UdpEchoClientReportHelper (Address ip, uint16_t port);
  /**
   * Create UdpEchoClientReportHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  UdpEchoClientReportHelper (Address addr);
  ...
  /**
   * Given a pointer to a UdpEchoClientReport application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be UdpEchoClientReport).
   ...
   *
   * \param node The Ptr<Node> on which to create the UdpEchoClientReportApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the UdpEchoClientReportApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::UdpEchoClientReport on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpEchoClientReport will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
...
#endif /* UDP_ECHO_REPORT_HELPER_H */
